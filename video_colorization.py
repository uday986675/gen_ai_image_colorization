...
class VideoColorizerGUI:
    ...
    def setup_gui(self):
        self.root.title("Real-Time Video Colorizer")
        ...
        model_combo = ttk.Combobox(control_frame, textvariable=self.model_var, 
                                  values=list(self.app.models.keys()), state="readonly")
        model_combo.grid(row=0, column=1, padx=5)
        model_combo.bind('<<ComboboxSelected>>', self.on_model_change)
        
        # Source Selection
        ttk.Button(control_frame, text="Webcam", 
                  command=lambda: self.start_video_source('webcam')).grid(row=0, column=2, padx=5)
        ttk.Button(control_frame, text="Load Video", 
                  command=self.load_video_file).grid(row=0, column=3, padx=5)
        
        # Display FPS
        self.fps_label = ttk.Label(control_frame, text="FPS: 0.0")
        self.fps_label.grid(row=0, column=4, padx=20)
        
        # Video Display Area
        display_frame = ttk.Frame(self.root)
        display_frame.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
        
        # Original and Colorized frames
        self.original_label = ttk.Label(display_frame)
        self.original_label.grid(row=0, column=0, padx=10, pady=10)
        
        self.colorized_label = ttk.Label(display_frame)
        self.colorized_label.grid(row=0, column=1, padx=10, pady=10)
